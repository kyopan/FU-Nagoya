...
IPAddress pickBroker(const String& ssid) {
  if (ssid == "FU") return IPAddress(192, 168, 1, 2);
  if (ssid == "PRACHTSAALstudio") return IPAddress(192, 168, 178, 178);
...
void setLEDColor(uint8_t r, uint8_t g, uint8_t b) {
  ...
  sendDataPacketToU3(current_f_r, current_f_g, current_f_b, current_r_r, current_r_g, current_r_b,
                     current_pitch, current_yaw, autonomousMode);
}
...
void setTubeInitialColor(uint8_t r, uint8_t g, uint8_t b, uint8_t brightness) {
  ...
  for (int step = 0; step <= fadeSteps; step++) {
    ...
    sendDataPacketToU3(fade_r, fade_g, fade_b, fade_r, fade_g, fade_b, current_pitch, current_yaw,
                       autonomousMode);
  ...
  // 最終的な目標色を確実に設定
  sendDataPacketToU3(r, g, b, r, g, b, 0, 0,
                     autonomousMode);  // brightness を1 or 0に変更　isAutonomous
  setOnboardLEDColor(r, g, b);
}
...
// // 統合データパケット送信関数
// void sendDataPacketToU3(uint8_t f_r, uint8_t f_g, uint8_t f_b, uint8_t r_r, uint8_t r_g, uint8_t
// r_b, float pitch_deg, float yaw_deg, uint8_t brightness) {
//   // pitch: -60°〜+60° → 0〜255
...
//   r_r, r_g, r_b, pitch8, yaw8, brightness);
//   Serial.printf("DATA,F,%hhu,%hhu,%hhu,R,%hhu,%hhu,%hhu,P,%hhu,%hhu,B,%hhu\n", f_r, f_g, f_b,
//   r_r, r_g, r_b, pitch8, yaw8, brightness);
// }

// 統合データパケット送信関数
void sendDataPacketToU3(uint8_t f_r, uint8_t f_g, uint8_t f_b, uint8_t r_r, uint8_t r_g,
                        uint8_t r_b, float pitch_deg, float yaw_deg, uint8_t brightness) {
  // 重複送信防止：前回と同じデータなら送信しない
  static uint8_t last_f_r = 255, last_f_g = 255, last_f_b = 255;
  static uint8_t last_r_r = 255, last_r_g = 255, last_r_b = 255;
  ...
  bool led_same = (f_r == last_f_r && f_g == last_f_g && f_b == last_f_b && r_r == last_r_r &&
                   r_g == last_r_g && r_b == last_r_b);
  ...
  Serial1.printf("DATA,F,%hhu,%hhu,%hhu,R,%hhu,%hhu,%hhu,P,%.2f,%.2f,B,%hhu,H,%.2f\n", f_r, f_g,
                 f_b, r_r, r_g, r_b, pitch_deg, yaw_deg, brightness, enc_horizontal);
  Serial.printf("DATA,F,%hhu,%hhu,%hhu,R,%hhu,%hhu,%hhu,P,%.2f,%.2f,B,%hhu,H,%.2f\n", f_r, f_g, f_b,
                r_r, r_g, r_b, pitch_deg, yaw_deg, brightness, enc_horizontal);
}
...
void getMacAddress() {
  uint8_t mac[6];
  WiFi.macAddress(mac);
  sprintf(macWithoutColons, "%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3], mac[4],
          mac[5]);
