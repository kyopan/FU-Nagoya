...
| **GY85 Ring** | IMUセンサ + 照明 | 24個 | GY-85 (6軸IMU) | 3.3V 100mA | 6本 |
| **合計** | - | **96個** | - | - | **24本** |
...
| 5V系 | Pitch Ring DCDC | 2A | RP2040・NeoPixel（全96個） |
...
| **NeoPixel** | 96個のLED制御 | シリアルデータチェーン（96個直列） |
...
| 2 | NeoPixel 24個 | 裏面、円環状配置 |
| 3 | DCDC 3.3V 100mA | 裏面 |
| 4 | 真鍮ワイヤーポート | Φ1mm対応、6個（円環LEDの内側に円環状配置） |
| 5 | 表面実装用ナット | 4個（対角・対称配置、重量調整用） |
...
| 4-1-2 | 　└ | NeoPixel 24個配置（裏面） | 鈴木さん | 10/20 | ⚪ Pending |
...
| 4-3-2 | 　└ | NeoPixel 24個配置（裏面） | 鈴木さん | 10/20 | ⚪ Pending |
